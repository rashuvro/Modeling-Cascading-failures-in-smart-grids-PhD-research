...
ParameterTable2V2;
%%%%%%%%%%%%%%%%%%%
Fend=zeros(length(Level),length(Alpha),length(Fi));
rate=zeros(length(Level),length(Alpha),length(Fi));
vari=zeros(length(Level),length(Alpha),length(Fi));
idx=0;
Dist=zeros(length(Level),length(Alpha),NumberOfLines);
tic
...
toc
save ('RateCombine2.mat','rate','Level','Alpha','-v7.3')